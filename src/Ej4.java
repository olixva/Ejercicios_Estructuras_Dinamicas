...
public class Ej4 {
    ...
    public void add(int indice, Object dato) {
        if (indice >= numElementos || indice < 0 ) throw new IndexOutOfBoundsException();

        if (numElementos == 0) {
        add(dato);  
        }else{
            
            Nodo nodoAnt = obtenerNodo(indice-1);
            Nodo nodoSig = obtenerNodo(indice-1);
            Nodo nuevo = new Nodo(dato);

            nodoAnt.siguiente = nuevo;
     ...
     * @exception indiceOutOfBoundsException - índice no está entre 0 y
     *                                       numElementos-1
     */
    ...
    public void removeAll(Ej4 datosBorrar){
        
        for (int i = 0; i < datosBorrar.size(); i++) {

            remove(datosBorrar.get(i));
     ...
     */
    public int indiceOf(Object dato) {
        Nodo actual = primero;
